...
import cl.magal.asistencia.ui.components.MonthSelector;
...
public class AttendancePanel extends VerticalLayout implements View {
	...
	BeanItemContainer<ConstructionSite> constructionContainer = new BeanItemContainer<ConstructionSite>(
	...
	Button btnExportSoftland, btnExportSupleSoftland, btnConstructionSiteConfirm, btnCentralConfirm,
			btnSupleObraConfirm, btnSupleCentralConfirm;
	Table confirmTable;
	LazyTabSheet tab;
	// VerticalLayout root;
	Table supleTable;
	...
	final static String[] defaultSalaryTableVisibleTable = new String[] { "laborerConstructionSite.jobCode",
			"laborerConstructionSite.laborer.fullname", "lastJornalPromedio", "jornalPromedio", "specialBond",
			"bondMov2", "loanBond", "sobreTiempo", "descHours", "loan", "tools", "totalLiquido" };
	final static String[] salaryTableVisibleTable = new String[] { "laborerConstructionSite.jobCode", // 1
			"laborerConstructionSite.laborer.fullname", // 2
			"costAccount", "lastJornalPromedio", // 3
			"jornalPromedio", // 4
			"specialBond", // 5
			"bondMov2", // 6
			"loanBond", // 7
			...
			"valorSabado", // 22
			"vsCorrd", // 23
			"descHoras", "bonifImpo", "glegal", "afecto", "sobreAfecto", "cargas", "asigFamiliar", "colacion", "mov",
			"mov2", "tnoAfecto" };

	final static String[] salaryTableVisibleTable2 = new String[] { "laborerConstructionSite.jobCode", // 1
			"laborerConstructionSite.laborer.fullname", // 2
			"costAccount.code", "lastJornalPromedio", // 3
			"jornalPromedio", // 4
			"specialBond", // 5
			"bondMov2", // 6
			"loanBond", // 7
	...
	final static String[] historicalSalaryTableVisibleTable = new String[] { "laborerConstructionSite.jobCode",
			"laborerConstructionSite.laborer.fullname", "costAccount.code", "lastJornalPromedio", "jornalPromedio",
			"specialBond", "bondMov2", "loanBond", "sobreTiempo", "descHours", "loan", "tools", "totalLiquido",
			"diaTrab", "sab", "sep", "dps", "dpd", "col", "mov", "jornalBaseMes", "vtrato", "valorSabado", "vsCorrd",
			"descHoras", "bonifImpo", "glegal", "afecto", "sobreAfecto", "cargas", "asigFamiliar", "colacion", "mov2",
			"tnoAfecto" };
	...
	private DateTime getAttendanceDate() {
		...
		// attendanceDate.setValue(new DateTime().withDayOfMonth(15).toDate());
		// //agrega el día 15
		// }
	...
	private DateTime getPastMonthClosingDate() {
		...
		if (dateConfig.getAssistance() == null) {
			...
			dateConfig.setAssistance(
					dt.minusMonths(1).withDayOfMonth(dt.minusMonths(1).dayOfMonth().getMaximumValue()).toDate());
		}
		return new DateTime(dateConfig.getAssistance());
	}
	...
	private void init() {
		...
		// ventana para mostrar el estado de carga de asistencia
		...
		// crea el indicador que mostará la ventana modal
	...
	public void enter(ViewChangeEvent event) {
		logger.debug("el event {}", event);
		if (event == null && cs == null) {
			logger.debug("el event y el cs son nulos");
			showErrorParam();
			return;
		} else if (event == null) {
			reloadAttendance();
			...
			logger.debug("recuperando parametros de la url {}", event.getParameters());
			...
			// si no viene fecha en la url entonces carga la por defecto
			if (msgs.length == 2) {
				attendanceDate.setValue(new Date());
			} else

			// si tiene más de 2 entonces trae el numero del mes por lo que lo
			// preseleccionar
			if (msgs.length > 2) {
				try {
					// intenta posicionar la asistencia en mes y año dado con el
					// formaro MMyyyy
					DateTime dt = DateTimeFormat.forPattern("MMyyyy").parseDateTime(msgs[2]);
					attendanceDate.setValue(dt.toDate());
				} catch (Exception e) {
					// si no es un número válido, lo ignora
	...
	private TabSheet drawAttendanceDetail() {
		...
		// agrega las confirmaciones solo si se tiene permiso para confirmar
		// central
	...
	private void enableOrDisableComponents() {
		...
		boolean attendanceEnabled = (SecurityHelper.hasConstructionSite(cs) && // tiene
																				// asociada
																				// la
																				// obra
				SecurityHelper.hasPermission(Permission.EDITAR_ASISTENCIA) && // tiene
																				// permisos
																				// para
																				// editar
																				// asistencia
				!getConfirmations().isConstructionSiteCheck() // si aún no se
																// confirma ese
																// mes
		) || // O
				SecurityHelper.hasPermission(Permission.DESBLOQUEDAR_ASISTENCIA); // si
																					// tiene
																					// permisos
																					// para
																					// cancelar
		...
		// la asistencia de obra se puede editar si
		boolean supleEnabled = (SecurityHelper.hasConstructionSite(cs) && // tiene
																			// asociada
																			// la
																			// obra
				SecurityHelper.hasPermission(Permission.EDITAR_ASISTENCIA) && // tiene
																				...
																				// editar
																				// asistencia
				!getConfirmations().isSupleObraCheck() // si aún no se confirma
														// ese mes
		) || // O
	...
	private void setVisibilityOfButtons() {
		// muestra el boton de obra, si tiene el permiso o si tiene permisos de
		// confirmación central y esta confirmado de obra (para que pueda
		// cancelar la confirmación)
		boolean showOnConstructionSite = SecurityHelper.hasPermission(Permission.CONFIRMAR_ASISTENCIA_OBRA)
				|| (SecurityHelper.hasPermission(Permission.CONFIRMAR_ASISTENCIA_CENTRAL)
						&& getConfirmations().isConstructionSiteCheck());

		boolean showOnCentral = SecurityHelper.hasPermission(Permission.CONFIRMAR_ASISTENCIA_CENTRAL);
		...
		// permite exportar a softland si se tiene doble confirmación y se tiene
		// permisos
		...
		/**
		 * Use el mismo código de arriba, reemplace los valores para considerar
		 * los botones del suple que se comportan igual.
		 */
		btnSupleObraConfirm.setVisible(SecurityHelper.hasPermission(Permission.CONFIRMAR_ASISTENCIA_OBRA)
				|| (SecurityHelper.hasPermission(Permission.CONFIRMAR_ASISTENCIA_CENTRAL)
						&& getConfirmations().isSupleObraCheck()));
	...
	private void toogleButtonState(Button btn, boolean confirmations) {
		if (confirmations) {
			...
			btn.setCaption(
					btn.getCaption().indexOf("Cancelar ") >= 0 ? btn.getCaption() : "Cancelar " + btn.getCaption());
			...
			btn.setCaption(btn.getCaption().indexOf("Cancelar ") >= 0 ? btn.getCaption().replace("Cancelar ", "")
					: btn.getCaption());
	...
	private void enableOrDisableButtons() {
		...
		btnConstructionSiteConfirm
				.setEnabled((getConfirmations().isConstructionSiteCheck() && !getConfirmations().isCentralCheck() // si
																													// está
																													// confirmado
																													// por
																													// obra
																													...
																													// por
																													// central
						&& SecurityHelper.hasPermission(Permission.CONFIRMAR_ASISTENCIA_CENTRAL) // y
																									// el
																									// usuario
																									// tiene
																									// permitido
																									...
																									// obra
																									// )
		) || !getConfirmations().isConstructionSiteCheck() // si no se ha
															// checkeado la
															// construcción
		);
		...
		btnCentralConfirm
				.setEnabled((getConfirmations().isConstructionSiteCheck() && !getConfirmations().isCentralCheck())
						|| (getConfirmations().isConstructionSiteCheck() && getConfirmations().isCentralCheck()
								&& SecurityHelper.hasPermission(Permission.DESBLOQUEDAR_ASISTENCIA)));

		// si está confirmado, lo habilita solo si tiene permiso para confirmar
		// asistencia central
		btnSupleObraConfirm
				.setEnabled((getConfirmations().isSupleObraCheck() && !getConfirmations().isSupleCentralCheck() // si
																												// está
																												// confirmado
																												// por
																												// obra
						...
						&& SecurityHelper.hasPermission(Permission.CONFIRMAR_ASISTENCIA_CENTRAL) // y
																									...
																									// obra
																									// )
		) || !getConfirmations().isSupleObraCheck());// si no se ha checkeado la
														// construcción);

		btnSupleCentralConfirm
				.setEnabled((getConfirmations().isSupleObraCheck() && !getConfirmations().isSupleCentralCheck())
						|| (getConfirmations().isSupleObraCheck() && getConfirmations().isSupleCentralCheck()
								&& SecurityHelper.hasPermission(Permission.DESBLOQUEDAR_ASISTENCIA)));

		btnExportSoftland.setEnabled(getConfirmations().isCentralCheck()); // sólo
																			// se
																			// habilita
																			// si
																			// está
	...
	private void createHeaders(final Grid grid) {
		...
		// si la propiedad comienza con d (dia) o dmp (dia mes pasado), entonces
		...
		for (final Object pid : grid.getContainerDataSource().getContainerPropertyIds()) {
			...
			if (pid.equals("laborerConstructionSite.jobCode")
					|| pid.equals("laborerConstructionSite.laborer.fullname")) {

				// solo define el filtro si no existe
				if (cell.getCellType() == GridStaticCellType.WIDGET && cell.getComponent() != null)
					continue;
				...
				// rutina para mostrar el nombre de la semana correspondiente y
				// si se debe o no mostrar la columna
			} else {
				...
				// TODO si hay que cambiar la fecha de sobretiempo, aqui se
				// deberia discriminar
				...
				if (((String) pid).startsWith("dmp") || ((String) pid).startsWith("dma")) {
					...
					// crea una fecha con el año y mes seleccionado y con el dia
					// recuperado del property actual
					DateTime dt2 = dt;
					try {
						// si el property es dmp(dia mes pasado, entonces
						// retrocede un mes
						if (((String) pid).startsWith("dmp"))
							dt2 = dt2.minusMonths(1);
						dt2 = dt2.withDayOfMonth(monthDay);
					} catch (Exception e) { // si no es un día válido del mes
											// válida lo ocula
						dt2 = null;
						if (grid.getColumn(pid) != null)
					...
					if (dt2 != null) { // continua solo si el día es válido
						...
						// si la grid es de asistencia, calcula el rango con el
						// dia de cierre del mes pasado más 1 y el ultimo dia
						// del mes seleccionado
						DateTime startDate, endDate;
						if (isAttendanceGrid) {
							startDate = getPastMonthClosingDate().plusDays(1);
							endDate = getAttendanceDate()
									.withDayOfMonth(getAttendanceDate().dayOfMonth().getMaximumValue());
						} else {// si la grid es de sobretiempos, calcula el
								// rango con el dia de inicio del trato y el fin
								// de trato
							startDate = new DateTime(getDateConfigurations().getBeginDeal());
							endDate = new DateTime(getDateConfigurations().getFinishDeal());
						}
						// define el rango
						Interval interval = new Interval(startDate, endDate.plusDays(1)); // le
																							// suma
																							// un
																							// dia,
																							// dado
																							...
																							// el
																							// final
						// si la fecha que representa el property está dentro
						// del rango, se preocupa de mostrarla y definir sus
						// header y subheader
						if (interval.contains(dt2)) {
							...
							grid.getColumn(pid).setHeaderCaption(((String) pid).replace("dmp", "").replace("dma", ""))
									.setSortable(false);
					...
					if (isAttendanceGrid) {
						// le entrega el converter a la columna para mostrar
						// null como empty
						if (grid.getColumn(pid) != null)
		...
		grid.setCellStyleGenerator(new Grid.CellStyleGenerator() {
			...
			public String getStyle(CellReference cellReference) {
				String post = "";
				if ((cellReference.getValue() instanceof AttendanceMark
						&& !AttendanceMark.ATTEND.equals(cellReference.getValue()))
				...
				if (pid.startsWith("dmp") || pid.startsWith("dma")) {
					...
					if (monthDay <= dt2.dayOfMonth().getMaximumValue()) {
						// si es el dia actual
						if (dt2.withDayOfMonth(monthDay).toString("dd/MM/yyyy")
	...
	private VerticalLayout drawSupleLayout() {
		salaryContainer.addNestedContainerProperty("laborerConstructionSite.jobCode");
		...
		VerticalLayout vl = new VerticalLayout() {
			{
						...
						btnValidate.addClickListener(new Button.ClickListener() {
							...
							public void buttonClick(ClickEvent event) {
								...
								if ((itemIds = checkHasNegativeOrNull(salaryContainer, "suple")).size() == 0) {
									Notification.show(
											"La lista de trabajadores no tiene valores negativos en el suple.",
											Type.HUMANIZED_MESSAGE);
								} else {
									// crea el mensaje
									StringBuilder sb = new StringBuilder(
											"Hay " + itemIds.size() + " trabajadores con valores negativos y son:\n");
									int i = 0;
										...
										"Continuar", "Cancelar", new ConfirmDialog.Listener() {
											public void onClose(ConfirmDialog dialog) {
												if (dialog.isConfirmed()) {
													...
													// si todo sale bien, manda
													// un email a los centrales
								...
								ConfirmDialog.show(UI.getCurrent(), "Confirmar Acción:",
										...
										"Continuar", "Cancelar", new ConfirmDialog.Listener() {
											public void onClose(ConfirmDialog dialog) {
												if (dialog.isConfirmed()) {
													confirmations
															.setSupleCentralCheck(!confirmations.isSupleCentralCheck());
													constructionSiteService.save(confirmations);
													toogleButtonState(btnSupleCentralConfirm,
															confirmations.isSupleCentralCheck());
				...
				supleTable.addGeneratedColumn("supleSection", new Table.ColumnGenerator() {
					...
					public Object generateCell(Table source, final Object itemId, Object columnId) {
						...
						btnSuple.addClickListener(new Button.ClickListener() {
							...
							public void buttonClick(ClickEvent event) {
								...
								synchronized (btnSuple) {
									...
									// lo pide explicitamente para obligar a
									// recalcular el suple
								...
								// guarda el trabajador, para guardar el codigo
								// de suple
						...
						((ValueChangeNotifier) beanItem.getItemProperty("suple"))
								.addValueChangeListener(new Property.ValueChangeListener() {

									@Override
									public void valueChange(ValueChangeEvent event) {
										// si el valor es distinto al suple que
										// le corresponde, entonces lo marca
										// como no calculado
										// y muestra el botón para volverlo al
										// por defecto

										// primero intenta transformarlo
										// Double value = (Double)
										// event.getProperty().getValue();
										// //luego busca el valor que le
										// corresponderia
										// Integer supleCode = (Integer)
										// beanItem.getItemProperty("laborerConstructionSite.supleCode").getValue();
										// if(supleCode == null ) // si aún no
										// se define el codigo de suple, se
										// retorna
										// return;
										// Double supleAmount = null;
										// for(AdvancePaymentItem item :
										// advancepayment.getAdvancePaymentTable()
										// ){
										// if(item.getSupleCode() == supleCode
										// );
										// supleAmount =
										// item.getSupleTotalAmount();
										// }
										//
										// //si no son iguales los valores
										// boolean sameValues = supleAmount !=
										// null && value != null &&
										// supleAmount.doubleValue() ==
										// value.doubleValue();
										synchronized (btnSuple) {
											Boolean b = (Boolean) btnSuple.getData();
										...
										// independiente de lo que pase, guarda
										// el nuevo suple
										// constructionSiteService.save(beanItem.getBean());
						...
						// if( ids.get(key) &&
						// key ==
						// beanItem.getBean().getLaborerConstructionSite().getLaborer().getId()
						// &&
				...
				supleTable.setVisibleColumns("laborerConstructionSite.jobCode",
						"laborerConstructionSite.laborer.fullname", "supleSection", "suple");
				...
				supleTable.setTableFieldFactory(new TableFieldFactory() {
					...
					public Field<?> createField(Container container, final Object itemId, Object propertyId,
						...
						if (propertyId.equals("laborerConstructionSite.jobCode")
						...
						if (propertyId.equals("suple")) {
							// tf.addValueChangeListener(new
							// Property.ValueChangeListener() {
							//
							// @Override
							// public void valueChange(ValueChangeEvent event) {
							// // logger.debug(" suple value changed ");
							// // BeanItem<Salary> beanItem =
							// salaryContainer.getItem(itemId);
							// //
	...
	private VerticalLayout drawSalaryLayout() {
		...
		salaryContainer.addNestedContainerProperty("salaryCalculator.print");
		...
		VerticalLayout vl = new VerticalLayout() {
			{
				...
				HorizontalLayout hl = new HorizontalLayout() {
					{
							...
							public void buttonClick(ClickEvent event) {
								...
								for (Long itemId : salaryContainer.getItemIds()) {
									...
									try {
										...
										logger.error(
												"Existe asistencia proyectada o real no definida, por lo que no se puede calcular el sueldo",
												e);
										Notification.show(
												"Existe asistencia proyectada o real no definida, por lo que no se puede calcular el sueldo.",
						...
						btnValidate.addClickListener(new Button.ClickListener() {
							...
							public void buttonClick(ClickEvent event) {
								List<Object> itemIds = null;
								if ((itemIds = checkHasNegativeOrNull(salaryContainer, "salary", "vtrato",
										"valorSabado", "vsCorrd", "glegal", "afecto", "colacion", "mov", "mov2"))
												.size() == 0) {
									Notification.show(
											"La lista de trabajadores no tiene valores negativos en el sueldo ni en el jornal promedio.",
											Type.HUMANIZED_MESSAGE);
								} else {
									// crea el mensaje
									StringBuilder sb = new StringBuilder(
						...
						btnValidarCostos.addClickListener(new Button.ClickListener() {
							...
							public void buttonClick(ClickEvent event) {
								...
								if (itemIds.size() == 0)
									Notification.show("Todos los trabajadores tienen asignada una cuenta de costos.",
											Type.HUMANIZED_MESSAGE);
								else {
									// crea el mensaje
									StringBuilder sb = new StringBuilder(
											"Hay " + itemIds.size() + " trabajadores sin cuenta de costos:\n");
									int i = 0;
									for (Object itemId : itemIds) {
										BeanItem<Salary> item = salaryContainer.getItem(itemId);
										sb.append(item.getBean().getLaborerConstructionSite().getJobCode());
							...
							public void buttonClick(ClickEvent event) {
								...
								ConfirmDialog.show(UI.getCurrent(), "Confirmar Acción:",
										...
										"Continuar", "Cancelar", new ConfirmDialog.Listener() {
											public void onClose(ConfirmDialog dialog) {
												if (dialog.isConfirmed()) {
													confirmations.setConstructionSiteCheck(
															!confirmations.isConstructionSiteCheck());
													constructionSiteService.save(confirmations);
													configureInterface();
													// si todo sale bien, envia
													// el mail a los centrales
										...
										"Continuar", "Cancelar", new ConfirmDialog.Listener() {
											public void onClose(ConfirmDialog dialog) {
												if (dialog.isConfirmed()) {
													...
													// copia los salarios y los
													// pasa a históricos
													List<HistoricalSalary> historicals = new ArrayList<HistoricalSalary>(
															salaryContainer.size());
													for (Long itemId : salaryContainer.getItemIds()) {
				...
				salaryTable.setItemDescriptionGenerator(new ItemDescriptionGenerator() {
						...
						if (propertyId != null) {
							if (propertyId.equals("laborerConstructionSite.laborer.fullname")) {
								...
								if (item != null && item.getBean() != null
										&& item.getBean().getLaborerConstructionSite() != null)
									return item.getBean().getLaborerConstructionSite().getStartDateString() + "-"
											+ item.getBean().getLaborerConstructionSite().getTerminationDate();
								else
									return "";
							} else if (propertyId.equals("salaryCalculator.dpd")) {
								return (String) salaryContainer.getItem(itemId)
										.getItemProperty("salaryCalculator.print").getValue();
							}
						}
						return null;
					}
				...
				salaryTable.addColumnCollapsedListener(new ColumnCollapsedObservableTable.ColumnCollapsedListener() {
					...
					public void colapseColumn(ColumnCollapsedEvent event) {
						...
						// agrega la columna al usuario si es que se setea como
						// no colapsada
						if (!event.isCollapsed() && !user.getSalaryColumns().contains(propertyId)) {
							user.getSalaryColumns().add(propertyId);
							user.setPassword(null);
							userService.saveUser(user);
						} else if (event.isCollapsed() && user.getSalaryColumns().contains(propertyId)) { // si
																											// no
																											// lo
																											// quita
				...
				salaryTable.setTableFieldFactory(new TableFieldFactory() {
					...
					public Field<?> createField(Container container, final Object itemId, Object propertyId,
						...
						if (propertyId.equals("laborerConstructionSite.jobCode")
								|| propertyId.equals("laborerConstructionSite.laborer.fullname")
								|| propertyId.equals("totalLiquido") || propertyId.equals("lastJornalPromedio")
								|| propertyId.equals("suple"))
							return null;
						if (propertyId.equals("jornalPromedio") || propertyId.equals("descHours")
						...
						if (propertyId.equals("costAccount")) {
							BeanItemContainer<CostAccount> costContainer = new BeanItemContainer<CostAccount>(
									CostAccount.class);
	...
	/**
	 * Permite verificar si un contenedor tiene valores nulos o menores a 0 para
	 * las propiedades dadas.
	 * 
	 * @param container
	 * @param propertyIds
	 * @return El primer itemId del item que tiene una propiedad nula o menor a
	 *         0
	 */
	private List<Object> checkHasNegativeOrNull(BeanContainer<Long, Salary> container, String... propertyIds) {
		...
		for (Object itemId : container.getItemIds())
			for (String propertyId : propertyIds) {
				...
				if (property == null || property.getValue() == null || ((Number) property.getValue()).intValue() < 0)
					itemdIds.add(itemId);
			}
		return itemdIds;
	}
	...
	/**
	 * Permite crear el footer de la tabla dada, sumando las propiedades de las
	 * columnas de tipo double o integer. Ignora las columnas de jobCode o
	 * fullname si las tuviera.
	 * 
	 * @param table
	 */
	private void createTableFooter(Table table) {
		...
		for (Object itemId : table.getContainerDataSource().getItemIds()) {
			...
			for (Object propertyId : table.getContainerPropertyIds()) {
				if (propertyId.equals("laborerConstructionSite.jobCode") || propertyId.equals("forceSalary")
		...
		for (Object propertyId : table.getContainerPropertyIds()) {
			if (propertyId.equals("laborerConstructionSite.jobCode") || propertyId.equals("forceSalary")
					|| propertyId.equals("forceSuple") || propertyId.equals("laborerConstructionSite.laborer.fullname"))
				continue;
		...
		table.sort(new Object[] { "laborerConstructionSite.jobCode" }, new boolean[] { true });
	}

	/**
	 * Tabla que contiene un listado de todas las posibles ausencias en el
	 * sistema (enfermedad, vacaciones, accidentes, etc) y las lista dando la
	 * posibilidad de modificar sus fechas de inicio/final o marcarla como
	 * confirmada en el caso que corresponda.
	 * 
	 * @return
	 */
	private Table drawAbsenceConfirmTable() {

		absenceContainer.addNestedContainerProperty("laborerConstructionsite.jobCode");
		...
		confirmTable.addGeneratedColumn("laborerConstructionsite.jobCode", new Table.ColumnGenerator() {
			@Override
		...
		confirmTable.setVisibleColumns("laborerConstructionsite.jobCode", "laborerConstructionsite.laborer.fullname",
				"type", "description", "fromDate", "toDate", "confirm");
	...
	private Grid drawOvertimeGrid() {
		...
		overtimeContainer.addNestedContainerProperty("laborerConstructionSite.jobCode");
		...
		// overtimeGrid.setEditorFieldGroup(new
		// BeanFieldGroup<Overtime>(Overtime.class));
		...
		// overtimeGrid.setEditorFieldGroup(new
		// BeanFieldGroup<EnhancedFieldGroupFieldFactory>(EnhancedFieldGroupFieldFactory.class));
		overtimeGrid.getEditorFieldGroup().addCommitHandler(new CommitHandler() {
			...
			public void postCommit(CommitEvent commitEvent) throws CommitException {
				...
				overtimeContainer.sort(new Object[] { "laborerConstructionSite.jobCode" }, new boolean[] { true });
		...
		overtimeContainer.sort(new Object[] { "laborerConstructionSite.jobCode" }, new boolean[] { true });
		overtimeGrid.getColumn("laborerConstructionSite.jobCode").setHeaderCaption("Rol")
				.setEditorField(new TextField() {
					{
						setReadOnly(true);
					}
				}).setWidth(50);
		overtimeGrid.getColumn("laborerConstructionSite.laborer.fullname").setHeaderCaption("Nombre")
				.setEditorField(new TextField() {
					{
						setReadOnly(true);
					}
	...
	private void setOvertimeOrders() {
		String[] s = new String[] { "laborerConstructionSite.jobCode", "laborerConstructionSite.laborer.fullname",
				"total", "dmp1", "dmp2", "dmp3", "dmp4", "dmp5", "dmp6", "dmp7", "dmp8", "dmp9", "dmp10", "dmp11",
				"dmp12", "dmp13", "dmp14", "dmp15", "dmp16", "dmp17", "dmp18", "dmp19", "dmp20", "dmp21", "dmp22",
				"dmp23", "dmp24", "dmp25", "dmp26", "dmp27", "dmp28", "dmp29", "dmp30", "dmp31", "dma1", "dma2", "dma3",
				"dma4", "dma5", "dma6", "dma7", "dma8", "dma9", "dma10", "dma11", "dma12", "dma13", "dma14", "dma15",
				"dma16", "dma17", "dma18", "dma19", "dma20", "dma21", "dma22", "dma23", "dma24", "dma25", "dma26",
				"dma27", "dma28", "dma29", "dma30", "dma31" };
	...
	private Grid drawAttendanceGrid() {
		...
		attendanceContainer.addNestedContainerProperty("laborerConstructionSite.jobCode");
		...
		bfg.addCommitHandler(new CommitHandler() {
			...
			public void postCommit(CommitEvent commitEvent) throws CommitException {
				...
				// si el día está fuera del rango del contrato, no puede ser
				// otra marca que R
				checkR(attendance);
			...
			/**
			 * Permite verificar si no se han seleccionado marcas distinas de R
			 * o vacia fuera de la fecha de contrato
			 * 
			 * @param attendance
			...
			private void checkR(Attendance attendance) throws CommitException {
				...
				while (Utils.isDateAfter(lc.getStartDate(), date.toLocalDate().toDate())
					...
					if (!Utils.isAttendanceMarkEmptyOrFilled(mark)) {
						throw new CommitException("El día " + Utils.date2String(date.toLocalDate().toDate())
								+ " está fuera del rango del contrato (" + Utils.date2String(lc.getStartDate()) + "-"
								+ Utils.date2String(lc.getTerminationDate()) + ") " + mark
								+ ", no puede tener una marca distinta a R o vacio (1).");
					}
					current++;
					if (current <= date.dayOfMonth().getMaximumValue())
						date = date.withDayOfMonth(current);
				}
				// rellena con R, todo lo que este fuera de la fecha final de
				// contrato
				if (lc.getTerminationDate() != null) {
					current = date.dayOfMonth().getMaximumValue();
					date = date.withDayOfMonth(current);
					while (Utils.isDateBefore(lc.getTerminationDate(), date.toLocalDate().toDate())
							&& current >= date.dayOfMonth().getMinimumValue()) {
						logger.debug("2 date {}", date);
						AttendanceMark mark = attendance.getMarksAsList().get(current - 1);
						boolean isSameWeek = Utils.isSameWeek(date, lc.getTerminationDate());
						// si es la misma semana, valida que tenga solo R, S y D
						// donde corresponda
						...
						if (isSameWeek) {
							...
							if (setHolidays.contains(date.toString("ddMMyyyy"))) {
								if (mark != AttendanceMark.SUNDAY)
									throw new CommitException("El día " + Utils.date2String(date.toLocalDate().toDate())
											+ " está fuera del rango del contrato ("
											+ Utils.date2String(lc.getStartDate()) + "-"
											+ Utils.date2String(lc.getTerminationDate()) + ") pero es feriado " + mark
											+ ", no puede tener una marca distinta a D.");
								// si el dia es sabado
								...
								if (!Utils.isBothAreSaturday(date, mark))
									throw new CommitException("El día " + Utils.date2String(date.toLocalDate().toDate())
											...
											+ Utils.date2String(lc.getStartDate()) + "-"
											+ Utils.date2String(lc.getTerminationDate()) + ") " + mark
											+ ", no puede tener una marca distinta a S.");
								...
								throw new CommitException(" El día " + Utils.date2String(date.toLocalDate().toDate())
										+ " está fuera del rango del contrato (" + Utils.date2String(lc.getStartDate())
										+ "-" + Utils.date2String(lc.getTerminationDate()) + ") " + mark
										+ ", no puede tener una marca distinta a R o F.");
						} else {// los demás dias sólo puede tener vacios
							if (!Utils.isAttendanceMarkEmpty(mark))
								throw new CommitException(" El día " + Utils.date2String(date.toLocalDate().toDate())
										+ " está fuera del rango del contrato (" + Utils.date2String(lc.getStartDate())
										+ "-" + Utils.date2String(lc.getTerminationDate()) + ") " + mark
										+ ", no puede tener una marca distinta vacio (2).");
						}
						current--;
				...
				// mientras la fecha de inicio sea mayor a la fecha recorrida
				while (Utils.isDateAfter(lc.getStartDate(), date2.toLocalDate().toDate())
						&& current <= date2.dayOfMonth().getMaximumValue()) {
					AttendanceMark mark = attendance.getLastMarksAsList().get(current - 1);
					if (!Utils.isAttendanceMarkEmptyOrFilled(mark))
						throw new CommitException("El día " + Utils.date2String(date2.toLocalDate().toDate())
								+ " está fuera del rango del contrato (" + Utils.date2String(lc.getStartDate()) + "-"
								+ Utils.date2String(lc.getTerminationDate()) + ") " + mark
								+ ", no puede tener una marca distinta a R o vacio (3).");
					current++;
				...
				if (lc.getTerminationDate() != null) {
					...
					date2 = date2.withDayOfMonth(current);
					logger.debug("4");
					while (Utils.isDateBefore(lc.getTerminationDate(), date2.toLocalDate().toDate())
							&& current >= date2.dayOfMonth().getMinimumValue()) {
						AttendanceMark mark = attendance.getLastMarksAsList().get(current - 1);
						boolean isSameWeek = Utils.isSameWeek(date2, lc.getTerminationDate());
						// si es la misma semana, valida que tenga solo R, S y D
						// donde corresponda
							...
							if (setHolidays.contains(date2.toString("ddMMyyyy"))) {
								if (mark != AttendanceMark.SUNDAY)
									throw new CommitException(
											"El día " + Utils.date2String(date2.toLocalDate().toDate())
													+ " está fuera del rango del contrato ("
													+ Utils.date2String(lc.getStartDate()) + "-"
													+ Utils.date2String(lc.getTerminationDate()) + ") pero es feriado "
													+ mark + ", no puede tener una marca distinta a D.");
								// si el dia es sabado
								...
								throw new CommitException("El día " + Utils.date2String(date2.toLocalDate().toDate())
										+ " está fuera del rango del contrato (" + Utils.date2String(lc.getStartDate())
										+ "-" + Utils.date2String(lc.getTerminationDate()) + ") " + mark
										+ ", no puede tener una marca distinta a R o F (4).");
						} else {// los demás dias sólo puede tener vacios
							if (!Utils.isAttendanceMarkEmpty(mark))
								throw new CommitException(" El día " + Utils.date2String(date2.toLocalDate().toDate())
										+ " está fuera del rango del contrato (" + Utils.date2String(lc.getStartDate())
		...
		attendanceGrid.setEditorErrorHandler(new EditorErrorHandler() {
			...
			public void commitError(CommitErrorEvent event) {
				List<InvalidValueException> invalidValues = new ArrayList<InvalidValueException>(
						event.getCause().getInvalidFields().values());
		...
		attendanceGrid.getEditorFieldGroup().addCommitHandler(new CommitHandler() {
			...
			public void postCommit(CommitEvent commitEvent) throws CommitException {
				// guarda el elmento
				Attendance attedance = ((BeanItem<Attendance>) commitEvent.getFieldBinder().getItemDataSource())
						.getBean();
				constructionSiteService.save(attedance);
				attendanceContainer.sort(new Object[] { "laborerConstructionSite.jobCode" }, new boolean[] { true });
				// por cada variable
		...
		attendanceContainer.sort(new Object[] { "laborerConstructionSite.jobCode" }, new boolean[] { true });
		attendanceGrid.getColumn("laborerConstructionSite.jobCode").setHeaderCaption("Rol").setWidth(50)
				.setEditorField(new TextField() {
					{
						setReadOnly(true);
					}
				});
		attendanceGrid.getColumn("laborerConstructionSite.laborer.fullname").setHeaderCaption("Nombre").setWidth(120)
				.setEditorField(new TextField() {
					{
						setReadOnly(true);
					}
	...
	private void createGridFooters(Grid grid) {
			...
			for (Object propertyId : grid.getContainerDataSource().getContainerPropertyIds()) {
				...
				if (attendanceItem.getBean() instanceof Attendance) {
					if (attendanceItem.getItemProperty(propertyId).getValue() instanceof AttendanceMark
							&& ((AttendanceMark) attendanceItem.getItemProperty(propertyId)
									.getValue() == AttendanceMark.ATTEND
									|| (AttendanceMark) attendanceItem.getItemProperty(propertyId)
											.getValue() == AttendanceMark.SATURDAY
									|| (AttendanceMark) attendanceItem.getItemProperty(propertyId)
											.getValue() == AttendanceMark.SUNDAY))
						counts[i]++;
		...
		for (Object propertyId : grid.getContainerDataSource().getContainerPropertyIds()) {
			...
			if (cell != null) {
				...
				// //calculo de la semana
				// if(((String) propertyId).startsWith("dmp") || ((String)
				// propertyId).startsWith("dma") ){
				// //calcula el numero del mes
				// int monthDay = Integer.parseInt(((String)
				// propertyId).replace("dmp","").replace("dma",""));
				// DateTime dt2 = dt;
				// if ( ((String) propertyId).startsWith("dmp") )
				// dt2 = dt2.minusMonths(1);
				// dt2.withDayOfMonth(monthDay);
				// //el número de mes no es un número válido de mes o si es un
				// día a la fecha de cierre del mes pasado, oculta la columna
				// if(dt2.getDayOfWeek() == DateTimeConstants.SATURDAY ||
				// dt2.getDayOfWeek() == DateTimeConstants.SUNDAY ){
				cell.setStyleName("red-color bold");
				// }else{ //solo lo setea si el número es mayor a la cantidad de
				// dias del mes
				// cell.setStyleName("");
				// }
				// }
	...
	String[] attendanceOrder = new String[] { "laborerConstructionSite.jobCode",
			"laborerConstructionSite.laborer.fullname", "dmp1", "dmp2", "dmp3", "dmp4", "dmp5", "dmp6", "dmp7", "dmp8",
			"dmp9", "dmp10", "dmp11", "dmp12", "dmp13", "dmp14", "dmp15", "dmp16", "dmp17", "dmp18", "dmp19", "dmp20",
			"dmp21", "dmp22", "dmp23", "dmp24", "dmp25", "dmp26", "dmp27", "dmp28", "dmp29", "dmp30", "dmp31", "dma1",
			"dma2", "dma3", "dma4", "dma5", "dma6", "dma7", "dma8", "dma9", "dma10", "dma11", "dma12", "dma13", "dma14",
			"dma15", "dma16", "dma17", "dma18", "dma19", "dma20", "dma21", "dma22", "dma23", "dma24", "dma25", "dma26",
			"dma27", "dma28", "dma29", "dma30", "dma31" };
	...
	Property.ValueChangeListener listener = new Property.ValueChangeListener() {
		...
		public synchronized void valueChange(ValueChangeEvent event) {
			...
			// cambia el valor de la url si es necesario
			String uri = Page.getCurrent().getUriFragment();
			String lastPart = new DateTime((Date) event.getProperty().getValue()).toString("MMyyyy");
			if (uri.contains("asistencia") && !(uri.endsWith(lastPart) || uri.endsWith(lastPart + "/"))) {
				StringBuilder sb = new StringBuilder();
				int indexOf = uri.indexOf("asistencia");
				sb.append(uri.substring(0, indexOf + 10)).append("/").append(lastPart);
				Page.getCurrent().setUriFragment(sb.toString(), false);
				reloadAttendance();
			}
			// si no cambio y el contenendor de asistencia sigue vacio, lo
			// intenta cargar
			else if (attendanceContainer.size() == 0) {
				reloadAttendance();
			}
	...
	private Command getCommandCarga() {
		return new Command() {
	...
	private Command getCommandSuple() {
		// TODO Auto-generated method stub
		return new Command() {

			@Override
			public void menuSelected(MenuItem selectedItem) {
				final BeanItemContainer<AdvancePaymentItem> container = new BeanItemContainer<AdvancePaymentItem>(
						AdvancePaymentItem.class);
				final Window window = new Window();
				...
				window.setContent(new VerticalLayout() {
					{
						...
						VerticalLayout form = new VerticalLayout() {
							{
								...
								final Property.ValueChangeListener listener = new Property.ValueChangeListener() {
									...
									public void valueChange(ValueChangeEvent event) {
										try {
											fg.commit();
											AdvancePaymentConfigurations bean = ((BeanItem<AdvancePaymentConfigurations>) fg
										...
										addComponent(new Panel("Configurar Suple", new VerticalLayout() {
											{
												...
												addComponent(new FormLayout() {
													{
														Field permissionDiscount = fg
																.buildAndBind("Descuento por Falla", "failureDiscount");
														((TextField) permissionDiscount).setNullRepresentation("");
														permissionDiscount.addValueChangeListener(listener);

														Field failureDiscount = fg.buildAndBind(
																"Descuento Adicional por Falla", "permissionDiscount");
												...
												HorizontalLayout hl = new HorizontalLayout() {
													{
														...
														Button add = new Button(null, new Button.ClickListener() {
															...
															public void buttonClick(ClickEvent event) {
																...
																AdvancePaymentConfigurations apItem = confService
																		.findAdvancePaymentConfigurationsByCS(cs);
																boolean check = false;
																try {
																	advancePaymentItem.setSupleCode(
																			Integer.valueOf(supleCode.getValue()));
																	for (AdvancePaymentItem i : apItem
																			.getAdvancePaymentTable()) {
																		if (i.getSupleCode() == advancePaymentItem
																				.getSupleCode())
																			check = true;
																	}
																	...
																	if (!check) {
																		...
																		advancepayment.addAdvancePaymentItem(
																				advancePaymentItem);
																		confService.save(advancepayment);
																		container.addBean(advancePaymentItem);
																	} else {
																		Notification.show(
																				"El código ingresado ya existe, ingrese uno diferente.",
																				Type.ERROR_MESSAGE);
																		return;
																	}
																} catch (Exception e) {
																	advancepayment.removeAdvancePaymentItem(
																			advancePaymentItem);
																	Notification.show(
																			"El código ingresado ya existe, ingrese uno diferente.",
																			Type.ERROR_MESSAGE);
												...
												table.addGeneratedColumn("eliminar", new Table.ColumnGenerator() {
													...
													public Object generateCell(Table source, final Object itemId,
														...
														return new Button(null, new Button.ClickListener() {
															...
															final AdvancePaymentItem advancePaymentItem = container
																	.getItem(itemId).getBean();

															@Override
															public void buttonClick(ClickEvent event) {
																List<LaborerConstructionsite> lcs = labcsRepo
																		.findByConstructionsiteAndIsActive(cs);
																boolean verify = false;
																for (LaborerConstructionsite lcsItem : lcs) {
																	if (lcsItem.getSupleCode() == advancePaymentItem
																			.getSupleCode()) {
																		verify = true;
																	}
																}
																...
																if (verify) {
																	Notification.show(
																			"El suple seleccionado no puede ser eliminado ya que está siendo utilizado.",
																			Type.ERROR_MESSAGE);
																} else {
																	ConfirmDialog.show(UI.getCurrent(),
																			"Confirmar Acción:",
																			"¿Está seguro de eliminar el suple seleccionado?",
																			"Eliminar", "Cancelar",
																				...
																				public void onClose(
																					...
																					if (dialog.isConfirmed()) {
																						try {
																							...
																							Notification.show(
																									"Error al quitar elemento",
																									Type.ERROR_MESSAGE);
																							logger.error(
																									"Error al eliminar un suple",
																									e);
																						}
																					}
																				}
												...
												table.addGeneratedColumn("supleTotalAmount",
														new Table.ColumnGenerator() {
															...
															public Object generateCell(Table source, Object itemId,
																...
																final BeanItem<?> item = (BeanItem<?>) source
																		.getItem(itemId);
																...
																Property.ValueChangeListener listener = new Property.ValueChangeListener() {
																	...
																	public void valueChange(
																		...
																		label.setValue(
																				((AdvancePaymentItem) item.getBean())
																						.getSupleTotalAmount()
																						.intValue() + "");
																	}
																};
																for (String pid : new String[] { "supleIncreaseAmount",
																		"supleNormalAmount" })
												...
												table.setVisibleColumns("supleCode", "supleTotalAmount",
														"supleNormalAmount", "supleIncreaseAmount", "eliminar");
												table.setColumnHeaders("Código Suple", "Monto Suple", "Normal",
														"Aumento Anticipo", "Eliminar");
						...
						btnGuardar.addClickListener(new Button.ClickListener() {
							...
							public void buttonClick(ClickEvent event) {
								...
								AdvancePaymentConfigurations apc = ((BeanItem<AdvancePaymentConfigurations>) fg
										.getItemDataSource()).getBean();
	...
	// crea el resource para descargar el excel
	OnDemandStreamResource myResource = new OnDemandFileDownloader.OnDemandStreamResource() {
		...
		public InputStream getStream() {
			...
			try {
				wb = new XSSFWorkbook(new FileInputStream(new File(
						AttendancePanel.class.getResource("/templates/asistencia/planilla_asistencia.xlsx").toURI())));
				...
				// celda B14 cuenta la cantidad dias de lunes a viernes
				// trabajados
				...
				for (int i = 101; i <= (101 + afpTable.getAfpTable().size()); i++) {
					...
					if (t < afpTable.getAfpTable().size()) {
						...
						row101.createCell(3).setCellValue(
								(afpTable.getAfpTable().get(t).getAfpAndInsuranceConfigurations().getSis() / 100));
					}
					t++;
				}
				...
				for (Object itemId : attendanceContainer.getItemIds()) {
					...
					// int minDay =
					// Utils.calcularDiaInicial(attendance,0,false);
					// int maxDay =
					// Utils.calcularDiaFinal(attendance,getAttendanceDate().dayOfMonth().getMaximumValue(),false)
					// + 1;
					// int minDay = 0;
					int maxDay = getAttendanceDate().dayOfMonth().getMaximumValue();// +
																					// 1;
					// asistencia
					int j = 3;
					...
					row.getCell(53).setCellValue(attendance.getLaborerConstructionSite().getStartDate());
					if (attendance.getLaborerConstructionSite().getTerminationDate() != null)
						row.getCell(54).setCellValue(attendance.getLaborerConstructionSite().getTerminationDate());
					// codigo suple
					...
					// solo agrega el mes anterior, si la fecha de inicio es en
					// el mes anterior a la fecha de fin de trato
					...
					row.getCell(152)
							.setCellValue(salary.getLaborerConstructionSite().getLaborer().getIsapre().toString());
					...
					Double rate = Utils.getAfpRate(afpTable.getAfpTable(),
							salary.getLaborerConstructionSite().getLaborer().getAfp());
					if (rate != 0)
		...
		private boolean validDateConfiguration(DateConfigurations dc) {
			...
			if (dc.getAdvance() == null) {
				Notification
						.show("No se puede generar el Excel si no se ha definido la fecha de fin del adelanto del mes");
				return false;
			}

			if (dc.getAssistance() == null) {
				Notification.show(
						"No se puede generar el Excel si no se ha definido la fecha de fin de asistencia del mes");
				return false;
			}

			if (dc.getBeginDeal() == null) {
				Notification
						.show("No se puede generar el Excel si no se ha definido la fecha de inicio de trato del mes");
				return false;
			}
		...
		return new HorizontalLayout() {
			{
				...
				btn.addClickListener(new ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						// cambia el valor de la url si es necesario
						String uri = Page.getCurrent().getUriFragment();
						String lastPart = monthSelector.getMonth();
						if (uri.contains("asistencia") && !(uri.endsWith(lastPart) || uri.endsWith(lastPart + "/"))) {
							StringBuilder sb = new StringBuilder();
							int indexOf = uri.indexOf("asistencia");
							sb.append(uri.substring(0, indexOf + 10)).append("/").append(lastPart);
						...
						// si no cambio y el contenendor de asistencia
						// sigue vacio, lo intenta cargar
						else if (attendanceContainer.size() == 0) {
							reloadAttendance();
						}
					}
				...
				// addComponent(new Label("Última Carga Información
				// Reloj: "));

				// agrega el menu de la derecha

				MenuBar menu = new MenuBar();
				addComponent(menu);
				setComponentAlignment(menu, Alignment.MIDDLE_RIGHT);

				MenuItem more = menu.addItem("", FontAwesome.BARS, null);
				// menu para cargar el archivo de asistencia
				MenuItem create = more.addItem("Cargar", FontAwesome.UPLOAD, getCommandCarga());
				// menu para configurar el suple
				MenuItem suple = more.addItem("Suple", FontAwesome.DOLLAR, getCommandSuple());
				// menu para exportar la asistencia a excel
				MenuItem export = more.addItem("Exportar", new Command() {
				...
				// {
				//
				// Button btnExportAttendance = new Button("Exportar a
				// Excel",FontAwesome.FILE_ARCHIVE_O);
				// addComponent(btnExportAttendance);
				//
				// OnDemandFileDownloader fileDownloader = new
				// OnDemandFileDownloader(myResource);
	...
	private void generateSoftlandFile(final Button btnExportSoftland) {
		...
		// StreamResource.StreamSource myResource = new
		...
		OnDemandStreamResource myResource = new OnDemandFileDownloader.OnDemandStreamResource() {
			...
			public InputStream getStream() {
				...
				// contrato
				sb.append(VelocityEngineUtils.mergeTemplateIntoString(velocityEngine, "templates/export/salary.vm",
						"UTF-8", input));
		...
		// StreamResource resource = new StreamResource(myResource,
		// getAttendanceDate().toString("YYYY_M_")+cs.getCode()+"_LIQ.txt");
	...
	private void generateSupleSoftlandFile(final Button btnExportSoftland) {
		...
		OnDemandStreamResource myResource = new OnDemandFileDownloader.OnDemandStreamResource() {
			...
			public InputStream getStream() {
				...
				// contrato
				sb.append(VelocityEngineUtils.mergeTemplateIntoString(velocityEngine, "templates/export/suple.vm",
						"UTF-8", input));
		...
		// StreamResource resource = new StreamResource(myResource,
		// getAttendanceDate().toString("YYYY_M_")+cs.getCode()+"_ANT.txt");
	...
	public void populateAttendanceGrid() {
		logger.debug("llamando populateAttendanceGrid");

		// final WorkThread thread = new WorkThread();
		// thread.start();
		DateTime dt = getAttendanceDate();
		// DateTime dt = new DateTime().withDate(2015, 12, 01);
		...
		// recupera las columnas de la tabla de sueldos seleccionadas del
		// usuario
		User user = SecurityHelper.getCredentials();
	...
	private void configureSalaryTable() {
			...
			salaryTable.addGeneratedColumn("totalLiquido", new Table.ColumnGenerator() {
				...
				public Object generateCell(final Table source, final Object itemId, final Object columnId) {
					...
					if (((BeanContainer) source.getContainerDataSource()).getBeanType() == Salary.class) {
						...
						return ((BeanItem<HistoricalSalary>) source.getContainerDataSource().getItem(itemId))
		...
		salaryTable.setColumnHeaders("Rol", // 1
				"Nombre", // 2
				"Cta. Costos", "Último<br />Jornal Prom", // 3
				"Jornal Prom", // 4
				"Bono Imp.", // 5
				"Bono No Imp.", // 6
				"Bono Prest.", // 7
				...
				"Valor Sábado", // 22
				"V S Corrida", // 23
				"Desc Horas", "Total<br />Bonos<br />Imponibles", "G Legal", "Afecto", "Sobre Afecto", "Cargas",
				"A Familiar", "Colación", "Mov", "Movi 2", "T No Afecto");
	...
	private void reloadMonthGridData(DateTime dt) {
		...
		loadingData = true;
		if (cs == null) {
			// throw new RuntimeException("No se ha seteado la obra en la vista
			// de asistencia.");
		...
		try {
			...
			absenceContainer.sort(new String[] { "laborerConstructionsite.jobCode" }, new boolean[] { true });

			// dependiendo de si está o no confirmada por el central, carga los
			// sueldos historicos u los otros
			if (!getConfirmations().isCentralCheck()) {
				List<Salary> salaries = constructionSiteService.getSalariesByConstructionAndMonth(cs, dt);
				salaryContainer.addAll(salaries);
				salaryContainer.sort(new String[] { "laborerConstructionsite.jobCode" }, new boolean[] { true });
				salaryTable.setContainerDataSource(salaryContainer);
				configureSalaryTable();
			} else {
				List<HistoricalSalary> salaries = constructionSiteService
						.getHistoricalSalariesByConstructionAndMonth(cs, dt);
				BeanContainer<Long, HistoricalSalary> historicalSalaryContainer = new BeanContainer<Long, HistoricalSalary>(
						HistoricalSalary.class);
				historicalSalaryContainer.addNestedContainerProperty("laborerConstructionSite.jobCode");
				...
				historicalSalaryContainer.sort(new String[] { "laborerConstructionsite.jobCode" },
						new boolean[] { true });
				...
				salaryTable.setColumnHeaders("Rol", "Nombre", "Cta. Costos", "Último<br />Jornal Prom", "Jornal Prom",
						"Bono Imp.", "Bono No Imp.", "Bono Prest.", "Sobretpo", "H Desc", "V Cuota<br />Prestamo",
						"V Cuota<br />Herramienta", "Total Líquido<br />(A Pagar)", "Día<br />Trab", "Sab", "Sep",
						"DPS", "DPD", "Col", "Mov", "Jornal Base", " V Trato", "Valor Sábado", "V S Corrida",
						"Desc Horas", "Total<br />Bonos<br />Imponibles", "G Legal", "Afecto", "Sobre Afecto", "Cargas",
						"A Familiar", "Colación", "Movi 2", "T No Afecto");
			}
			salaryTable.refreshRowCache();
	...
	/**
	 * Según la fecha y la obra, verifica cual es el estado de confirmación de
	 * cada una
	 */
	...
	/**
	 * Permite bloquear el ingreso de horas extras en caso de que un obrero
	 * registre vacaciones, accidente y/o licencia.
	 * 
	 * @param grid
	 */
		...
		for (Object itemId : grid.getContainerDataSource().getItemIds()) {
			...
			for (Object propertyId : grid.getContainerDataSource().getContainerPropertyIds()) {
				if (attendanceItem.getBean() instanceof Attendance) {
					if (attendanceItem.getItemProperty(propertyId).getValue() instanceof AttendanceMark
							&& ((AttendanceMark) attendanceItem.getItemProperty(propertyId)
									.getValue() == AttendanceMark.VACATION
									|| (AttendanceMark) attendanceItem.getItemProperty(propertyId)
											.getValue() == AttendanceMark.ACCIDENT
									|| (AttendanceMark) attendanceItem.getItemProperty(propertyId)
											.getValue() == AttendanceMark.SICK)) {
